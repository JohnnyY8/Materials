...
class SaliencyDetector:
  ...
  def cal_gradient(self):
    # Construct the grid
    index_start = eval(input("Please input the start index: "))
    index_end = index_start + self.ins_dataprocess.num_atoms
    length = self.ins_dataprocess.num_atoms * self.FLAGS.num_directions
    grid = self.ins_dataprocess.data[index_start, index_end].reshape(1, length)
    ...
    print("The saliency on x aiex:", select_max_gradient(gradient_x_x))
    gradient_y_y = (f_delta_y - f_all[:, 1]) / delta
    print("The saliency on y aiex:", select_max_gradient(gradient_y_y))
    gradient_z_z = (f_delta_z - f_all[:, 2]) / delta
    print("The saliency on z aiex:", select_max_gradient(gradient_z_z))


  def run_model(self, batch_x, batch_y):
    feed_data = {self.x_data: batch_x, self.y_label: batch_y, self.keep_prob: 1.0}
    temp_z = sess.run(self.z_output, feed_dict = feed_data)

    return temp_z


  def select_max_gradient(gradient):
